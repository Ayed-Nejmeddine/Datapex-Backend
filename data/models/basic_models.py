...
class Clean_Document(models.Model):
    """Clean Document model"""

    cleaned_document_path = models.FileField(
        blank=False, null=False, validators=[FileExtensionValidator(allowed_extensions=["csv"])]
    )
    owner = CurrentUserField()
    cleaning_date = models.DateTimeField(default=datetime.now())
    size = models.DecimalField(max_digits=9, decimal_places=3, null=True, blank=True)
    original_document = models.OneToOneField(Document, on_delete=models.CASCADE)
    objects = models.Manager()

    def __str__(self):
        return self.cleaned_document_path.name
